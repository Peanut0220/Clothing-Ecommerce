...
<jsp:include page="/CustGetStatus" />
<% List<domain.Orders> orderStatusList = (List<domain.Orders>) session.getAttribute("orderStatusList");%>
    ...
    <body>
        ...
        <div class="product-container">
            ...
            <div class="table-container">
                <p>Total : <%= orderStatusList.size()%></p>
                    ...
                    <tbody>
                        <%
                            for (int i = 0; i < orderStatusList.size(); i++) {
                                domain.Orders orders = orderStatusList.get(i);
                                SimpleDateFormat dt = new SimpleDateFormat("yyyy-MM-dd");
                                Date date = orders.getOrderdate();
                                Date date2 = orders.getEstimdate();
                        %>
                        <tr>
                            <td><%=(i + 1)%></td>
                            <td><%= dt.format(date)%></td>
                            <td><%= dt.format(date2)%></td>
                            <td><%=orders.getTrackingnum()%></td>
                            <td><%=orders.getOrderid()%></td>
                            <td>RM<%=orders.getTotal()%></td>
